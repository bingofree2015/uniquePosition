...
#define DCHECK(x)                           \
  {                                         \
    if (!x)                                 \
      LOG_ERROR << " dcheck error" << endl; \
  }

#define DCHECK_OP(name, op, val1, val2)                                  \
  {                                                                      \
    bool test = val1 op val2;                                            \
    if (!test)                                                           \
      LOG_ERROR << "failed dcheck: " << #val1 " " #op " " #val2 << endl; \
  }
...
{
  return chars[start_index] == chars[start_index + 1] && chars[start_index] == chars[start_index + 2] && chars[start_index] == chars[start_index + 3];
}
